...
| `build.py` | збирає мод у `build/uk-UA/`: `source/` з підставленими перекладами зі `strings.tsv` (лише файли, де є переклад), перекладені файли з `translation/files/`, `assets/`; `--install` / `--uninstall` — покласти в `<гра>/mods/uk-UA/` або прибрати |
