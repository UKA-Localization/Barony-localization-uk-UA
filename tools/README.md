...
| `build.py` | збирає мод у `build/Barony Ukrainian Localization/`: `source/` з підставленими перекладами зі `strings.tsv` (лише файли, де є переклад), книги зі `strings.tsv` (`books/*.txt`, CRLF), перекладені файли з `translation/files/`, `assets/`; `--install` / `--uninstall` — покласти в `<гра>/mods/Barony Ukrainian Localization/` або прибрати |
