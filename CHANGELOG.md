# Changelog

Формат — [Keep a Changelog](https://keepachangelog.com/uk/1.1.0/). Версії локалізації — `<версія-гри>-<номер-релізу>`, напр. `5.0.2-1`.

## [Unreleased]

## [5.0.2-3] — 2026-09-19

### Added
- Шрифти: відсутні символи `« » – — № ~` і комбінований наголос в усіх 9 шрифтах; скрипти `tools/check_font_symbols.py`, `extend_font_symbols.py`, `review_font_symbols.py`.
- `publish/`: скриншоти з гри, оновлені описи Steam (uk/en) — покрокове встановлення з повними шляхами, відомі проблеми, «Що нового»; картка для КУЛІ.

### Changed
- Репозиторій переїхав до організації UKA-Localization; дошка проєкту з відомими проблемами.

## [5.0.2-2] — 2026-09-17

### Added
- Чернетки перекладу всіх 34 книг; книги тепер у `strings.tsv` (`books/<назва>.txt`, один рядок на книгу), а не в `translation/files/`.
- Weblate: книги — один компонент `books`.

### Changed
- Тека мода — `Barony Ukrainian Localization` (замість `uk-UA`): так мод називається в списку Local Mods і в архіві релізу.

## [5.0.2-1] — 2026-09-17

Перший реліз: повна чернетка перекладу для Barony v5.0.2. Усі рядки мають `status=fuzzy` — не вичитано; книги не перекладено.

### Added
- Чернетки перекладу для 17 644 рядків (інтерфейс, предмети, монстри, закляття, компендіум, досягнення, сюжет, святкові теми).
- Глосарій: 2 297 термінів, усі затверджені; українські скорочення характеристик (ОЗ, ОМ, ОД, СИЛ, СПР, СТА, ІНТ, ЧУТ, ХАР, КЗ, АТК…).
- `publish/` — обкладинка мода й описи для Steam Workshop (uk/en) та GitHub Releases.
- `build.py --drafts` — включити чернетки в мод; компактний JSON і перевірка лімітів буферів гри (без цього гра падала на `callout_wheel.json`).
- `build.py --install` копіює поверх, а не видаляє теку (гра блокує шрифти, коли запущена).
- Скелет репозиторію.
- Тека `assets/` — оверлей теки гри (шлях у репо = шлях у грі).
- Шрифти гри з кириличними гліфами (`assets/fonts/`, 9 файлів).
- Оригінальні тексти гри v5.0.2 (`source/`, 104 файли), `translation/strings.tsv` (19 171 рядок) і `translation/files/` (38 файлів: книги, списки імен).
- `tools/extract.py` — витягування рядків із файлів гри зі збереженням наявних перекладів.
- `translation/glossary.tsv` як джерело глосарію і `tools/glossary.py`, що генерує з нього `GLOSSARY.md` та експорт для Weblate.
- `tools/weblate.py` — синхронізація `strings.tsv` ⇄ PO-файли в гілці `weblate` для Weblate (компонент на файл гри + глосарій).
- `tools/build.py` — збірка мода `build/Barony Ukrainian Localization/` та встановлення в `mods/` гри.
- `tools/validate.py` — перевірка плейсхолдерів у перекладах.
- CI (GitHub Actions): глосарій, плейсхолдери, актуальність `translation/`, збірка мода.
- Колонки `status` (непорожній — чернетка: не в мод, у Weblate — needs editing), `context` (опис рядка), `entities` (терміни глосарію в рядку) і `tone` (характер рядка) у `strings.tsv`; у `glossary.tsv` — `id` і `status`.
- `translation/STYLE.md` — правила стилю; `translation/pipeline.toml` — технічний опис файлів проєкту.
